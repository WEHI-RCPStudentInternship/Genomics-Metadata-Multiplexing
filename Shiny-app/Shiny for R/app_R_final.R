...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # Button
      h5("Download the following format as CSV file"),
      downloadButton("downloadData", "Download CSV file", style = "vertical-align: middle; width: 80%;"),
      ),
...
server <- function(input, output) {
  ...
  output$downloadData <- downloadHandler(
    filename = function() {
      paste("merged.csv", sep = "")
    },
    content = function(file) {
      if (input$mySelect == "Scale values") { write.csv(contents1(), file, row.names = FALSE) }
      else { write.csv(contents2(), file, row.names = FALSE) }
    }
  )
